...
# A density plot is generated for all samples with the same mark for the peak width distribution.
...
if (length(args) < 3) {
    stop("Usage: Rscript plot_peak_stats.R <peak_bed_dir> <sample_matrix_csv> <output_dir> [<plot_width>] [<plot_height>]")
}
...
for (mark in marks) {
    ...
    # Summary of the peak distribution for all samples with the same mark
    ...
    peakCountPlot <- ggplot(peakCountData, aes(x = Sample, y = PeakCount)) +
        ...
        theme_bw()
    pdfFile <- paste0(outputDir, "/", mark, "_peak_count_bar.pdf")
    ...
    cat("Plotting only peak widths less than ", maxPeakWidth90, "for mark", mark, ".\n")
    peakWidthPlot <- ggplot(peakWidthData, aes(x = PeakWidth, color = Sample)) +
        ...
        theme(axis.text.x = element_text(size = 14),
            ...
            axis.title.y = element_text(size = 16)) +
        theme_bw()
    pdfFile <- paste0(outputDir, "/", mark, "_peak_width_density.pdf")
    pdf(pdfFile, width = plotWidth, height = plotHeight)
    print(peakWidthPlot)
    dev.off()

    write.csv(summaryDist, paste0(outputDir, "/", mark, "_peak_width_summary.csv"), row.names = FALSE)
}
