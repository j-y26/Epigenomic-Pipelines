...
for file in $(find ${fastqDir} -type f -name '*_R1_*.fastq.gz'); do
    ...
    --unpaired1 ${trimmedDir}/unpaired/${sample}_R1_unpaired.fastq.gz \
    --unpaired2 ${trimmedDir}/unpaired/${sample}_R2_unpaired.fastq.gz \
    --failed_out ${trimmedDir}/filtered_out/${sample}_filtered_out.fastq.gz \
    -w ${threads} \
